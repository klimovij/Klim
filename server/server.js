...
// CORS origins - поддержка внешних доменов через переменную окружения
...
// Добавляем внешний IP/домен если указан
if (process.env.EXTERNAL_URL) {
  const externalUrl = process.env.EXTERNAL_URL.trim();
  if (!CORS_ORIGINS.includes(externalUrl)) {
    CORS_ORIGINS.push(externalUrl);
    // Также добавляем версию с http и https
    if (externalUrl.startsWith('http://')) {
      CORS_ORIGINS.push(externalUrl.replace('http://', 'https://'));
    } else if (externalUrl.startsWith('https://')) {
      CORS_ORIGINS.push(externalUrl.replace('https://', 'http://'));
    }
  }
}
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    if (CORS_ORIGINS.includes(origin) || CORS_ORIGINS.includes('*')) {
      ...
      // В production можно закомментировать эту строку для строгой проверки
      callback(null, true); // Временно разрешаем все для упрощения развертывания
...
// ==================== MIDDLEWARE ====================
// CORS уже настроен выше, дубликат удален

// Статическая раздача файлов
app.use('/uploads', express.static(path.join(__dirname, '../uploads')));

// --- СТАТИКА ЭМОДЗИ ---
// Путь к эмодзи: сначала пробуем относительный путь, затем абсолютный
const emojiDir = path.resolve(__dirname, '../client-react/src/assets/icons/Smile');
if (fs.existsSync(emojiDir)) {
  app.use('/emojis', express.static(emojiDir));
  console.log('✅ Emoji directory found:', emojiDir);
} else {
  // Fallback для Windows разработки (если нужно)
  const windowsEmojiDir = path.resolve('C:/Users/Ksendz/web/mesendger(самый удачный)/mesendger/telegram-clone/client-react/src/assets/icons/Smile');
  if (fs.existsSync(windowsEmojiDir)) {
    app.use('/emojis', express.static(windowsEmojiDir));
    console.log('✅ Emoji directory found (Windows):', windowsEmojiDir);
  } else {
    console.log('⚠️  Emoji directory not found, custom emojis may not work');
  }
}
...
app.post('/api/documents/upload', authenticateToken, docsUpload.single('file'), (req, res) => {
  try {
    ...
    const safeName = String(req.body?.name || req.file.originalname).trim();
    ...
    db.db.run(
      ...
      [safeName, req.file.originalname, req.file.mimetype, req.file.size, relPath, userId],
      function(err){
        if (err) return res.status(500).json({ success:false, error: err.message });
        return res.json({ success:true, document: { id: this.lastID, name: safeName, original_name: req.file.originalname, mime: req.file.mimetype, size: req.file.size, path: relPath, user_id: userId } });
      }
    );
  } catch(e){
...
// Генерация текста поздравления (заглушка)
app.post('/api/ai/generate', async (req, res) => {
  try {
    const { name, occasion } = req.body;
    const apiKey = GEMINI_API_KEYS[0];
    if (!apiKey) return res.status(500).json({ error: 'Нет API-ключа Gemini' });

    const prompt = `Сгенерируй красивое поздравление для сотрудника по имени ${name || 'Сотрудник'} по поводу ${occasion || 'дня рождения'} с эмодзи, обращением по имени и пожеланиями.`;
    ...
    const text = response.data?.candidates?.[0]?.content?.parts?.[0]?.text || 'Поздравление не удалось сгенерировать.';
    res.json({ text });
  } catch (error) {
    // Выводим подробную ошибку Gemini
    const geminiError = error?.response?.data || error?.message || error;
    console.error('❌ /api/ai/generate Gemini error:', geminiError);
    res.status(500).json({ error: 'Ошибка генерации поздравления через Gemini', details: geminiError });
  }
});
