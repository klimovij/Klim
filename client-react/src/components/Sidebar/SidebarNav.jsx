...
export default function SidebarNav() {
  ...
  useEffect(() => {
    ...
    const onCloseAll = () => closeAllModals();
  ...
  const fetchMyTasksCount = useCallback(() => {
    ...
    const apiBaseURL = typeof window !== 'undefined' && window.location ? window.location.origin : 'http://localhost:5000';
    fetch(`${apiBaseURL}/api/tasks/open`, {
  ...
  const handleNavClick = (n) => {
    setActive(n.key);
    // Сначала глобально закрыть все модалки и локально сбросить флаги
    try { window.dispatchEvent(new Event('close-all-modals')); } catch {}
    closeAllModals();
    // Не переключаем основной контент для модалок, чтобы не было дублирования страницы под модалкой
    // (Задачи, Общий календарь, Новости открываются во всплывающих окнах)
    if (n.key !== 'leaves' && n.key !== 'tasks' && n.key !== 'all-leaves' && n.key !== 'news' && n.key !== 'todo') {
    ...
    switch (n.key) {
      case 'all-leaves':
        fetchPendingCount();
        setTimeout(() => setShowGeneralCalendar(true), 0);
        break;
      case 'leaves':
        setTimeout(() => setShowLeaveCalendar(true), 0);
        break;
      case 'news':
        setTimeout(() => setShowNewsModal(true), 0);
        dispatch({ type: 'RESET_UNREAD_NEWS' });
      ...
      case 'employees':
        ...
        if (userRole === 'user') {
          ...
          // Админы и HR видят полную модалку с функциями сотрудников
          setTimeout(() => setShowBirthdaysModal(true), 0);
        }
        break;
      case 'tasks':
        setTimeout(() => setShowTasksModal(true), 0);
        setMyTasksCount(0);
        setTimeout(() => fetchMyTasksCount(), 300);
        break;
      case 'todo':
        setTimeout(() => setShowTodoModal(true), 0);
        break;
      case 'chats':
  ...
  return (
    <>
      ...
      {createSafePortal(
        <AllLeavesCalendar key={`all-leaves-${portalKey}`} open={showGeneralCalendar} onClose={()=>setShowGeneralCalendar(false)} token={localStorage.getItem('token')} />,
        showGeneralCalendar
      )}
